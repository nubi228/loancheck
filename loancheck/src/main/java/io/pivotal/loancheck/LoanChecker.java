...
public class LoanChecker {
  ...
  public void checkAndSortLoans(Loan loan) {
    log.info("{} order {} for ID{} from {}", loan.getStatus(), loan.getUuid(), loan.getAmount(), loan.getName());

    if (loan.getUuid().matches("EKS")) {
      loan.setStatus(Statuses.STARTING.name());
      processor.approved().send(message(loan));
    }else {
      loan.setStatus(Statuses.REJECTED.name());
